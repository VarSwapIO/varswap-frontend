...
export const send_message = async (url_command: string, payload: any, onload: () => void, onsuccess: (block: string | undefined) => void, onerror: () => void, keyPair: AccountSigner, sails: SailsCalls): Promise<any> => {
    const response = await sails?.command(
        ...
        keyPair,
        {
            ...
            callbacks: {
                ...
                onBlock(block: `0x${string}` | undefined) {
                    onsuccess?.(block)
                },
                onError() {
                    onerror?.();
